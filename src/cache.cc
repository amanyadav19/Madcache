...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            if(cache_type == IS_LLC) {
                if(set < LLC_TRACKER_SET && block[set][way].PC_pred_table_index != -1) {
                    pc_predictor[block[set][way].PC_pred_table_index].counter = max(0, pc_predictor[block[set][way].PC_pred_table_index].counter -1);
                }
            }
            if (cache_type == IS_LLC) {
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                if (cache_type == IS_LLC) {
                    way = lru_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type);
                }
                else
