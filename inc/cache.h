...
#define LLC_TRACKER_SET LLC_SET/16
#define PC_PREDICTOR_SIZE 1024

class PC_Predictor_Block {
public:
    bool policy;
    uint64_t pc;
    uint8_t counter; // only 6 bits of it are considered
    uint16_t num_entries; // only 9 bits are considered
    bool valid;
};
...
class CACHE : public MEMORY {
  public:
    ...
    // for LLC bypassing
    PC_Predictor_Block pc_predictor[PC_PREDICTOR_SIZE] ;
    uint16_t set_dueling_counter; // only 10 bits considered
	...
	total_miss_latency = 0;
        ...
        pf_useless = 0;
        pf_fill = 0;
        set_dueling_counter = (1<<9) - 1;
        for (int i = 0; i < PC_PREDICTOR_SIZE; i++) {
            pc_predictor[i].valid = false;
        }
    };
